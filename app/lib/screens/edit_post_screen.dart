...
class _EditPostScreen extends State<EditPostScreen> {
  ...
  setBody() async {
    Timer(const Duration(milliseconds: 650), () async {
      await controller.setText(body);
    });
  }

  void getData() async {
    Post post1 = await posts.getPostByIdPrivate(widget.postId);
    //await controller.setText(body);
    setState(() {
      post = post1;
      name = post.name;
  ...
  void postData() async {
    ...
    try {
      ...
      String res = await posts.updatePost(
          name, description, body, tags, url, widget.postId, draft);
